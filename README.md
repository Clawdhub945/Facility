...
2. **额外产品下拉框**：文案下方注入一个真正的下拉框（标题条 + 候选列表），
   点标题条展开，点哪项选哪项，选择直接写入 cfg 并热生效
   （候选清单由 cfg「额外产品候选」配置，格式 物品id:名称）

> 实现注记：游戏里原生 Unity `Dropdown` 点不开（控件可见、射线命中自身、
> 就是弹不出列表——游戏用手写输入派发）。所以下拉框是**自己拼的**：
> `Button.onClick` + `EventTrigger(PointerClick)` 双路挂接，
> 并用帧号去重（`ExecuteEvents` 会把两条回调都触发，不去重会「展开又收起」）。

## 建筑外观（预制体）

外观由 `Defs/stuff.json` 的 `prefab` / `stuff_img` / `stuff_img_on_map` 三个字段配套决定：

```bash
python _tools/make_defs.py --appearance 制造台 --deploy   # 采集营地/磨坊/交易台/制造台/熔炉
python _tools/test_appearance.py                          # 自动逐个试：能否进档 + 模型像素对比
```

默认外观 = **制造台**（`workbench`，产线建筑，最贴「综合生产所」）。
5 种预设实测都能正常进档，`goName` 随预设变化、建筑区像素差 6~8%（模型确实换了）。
改完需**重新进档**生效（Defs 在进档时读）。
